...
int main(int argc, char *argv[]) {
    ...
    //     constraint sets
    std::vector<std::string> constraintSetNames = Addons::LuaModelGetConstraintSetNames(modelFile.c_str());
    std::vector<RigidBodyDynamics::ConstraintSet> constraintSets;

    constraintSets.resize(constraintSetNames.size());

    if (!Addons::LuaModelReadFromFileWithConstraints(modelFile.c_str(), &model, constraintSets, constraintSetNames, false)) {
    ...
    controlled_stepper_type controlled_stepper(default_error_checker<double, range_algebra, default_operations>(absTolVal, relTolVal, a_x, a_dxdt));
    ...
    for (int i = 0; i <= nPts; i++) {
        ...
        integrate_adaptive(controlled_stepper, rbdlModel, xState, tp, t, (t - tp) / 10000);
        tp = t;

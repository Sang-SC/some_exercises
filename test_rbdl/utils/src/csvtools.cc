...
std::vector<std::vector<double> > readMatrixFromFile(const std::string& filename) {
    ...
    if (datafile.is_open()) {
        ...
        do {
            ...
            if (pos2 != std::string::npos && row == 0) matrixColNum++;
        ...
        while (datafile.good()) {
            ...
            for (int i = 0; i < matrixColNum; i++) {
                pos2 = line.find(",", pos1);
                if (pos2 == std::string::npos) pos2 = line.length();
...
void printMatrixToFile(const std::vector<std::vector<double> >& dataMatrix, const std::string& header, const std::string& filename) {
    std::ofstream datafile;
    datafile.open(filename.c_str());
    datafile << std::scientific;
    datafile.precision(16);
    if (header.length() > 1) datafile << header << "\n";
